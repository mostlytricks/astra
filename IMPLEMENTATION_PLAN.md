...
35 system tests against a temp registry: read surfaces (catalog, detail, `latest` alias/redirect, install-verification checklist), download byte-identity, every publish wall (token, disabled-without-token, immutability/409, missing SKILL.md, name mismatch, bad name/version, zip-slip, non-zip body), yank/unyank (hides from latest but keeps pins installable, bytes untouched, api/page flags, fully-yanked catalog state, token wall, unknown-version 404, unyank restores), and the bundle contract (console-nonascii is a publish-blocking error, non-stdlib import is a non-blocking warn, path-traversal rejected, SKILL.md exempt from ASCII, `/api/validate` dry-run needs no token). UI look stays `[review]`.

Last green: 2026-07-05 (35 passed).
