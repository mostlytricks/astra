---
name: astra-curate
description: Review an externally-authored agent skill for safety, prompt-injection, secrets, license, and contract compliance before it enters the astra registry — the curator's quality gate. On the curator's explicit approval, publishes it via the astra publish API. Curator use only (needs the publish token).
---
...
- `ASTRA_URL` — the registry base URL (e.g. `http://localhost:8300`). Needed for publish.
...
If an env var is missing, you can still review — just say publish will need it.
...
5. **Contract & hygiene** — `SKILL.md` at the folder root; frontmatter has kebab-case
   `name:` and a non-empty `description:`; no path-traversal entries (`..`, absolute
   paths); a sane, reviewable file set (flag large or opaque binaries you can't inspect);
   folder shape matches what Claude Code loads.
...
astra-curate · <skill-name>  (proposed v<X.Y.Z>)
  ...
  contract         PASS | FLAG | BLOCK   — <one line>
  ─────────────────────────────────────
...
Only after an explicit yes and with no unresolved BLOCK. (Same contract as the
`astra-publish` skill; if that skill is installed you may defer to it.)
...
2. **Zip the folder's contents** (`SKILL.md` at the zip root, not the folder itself):

   ```powershell
   Compress-Archive -Path <folder>\* -DestinationPath "$env:TEMP\<name>.zip" -Force
   ```

3. **POST it** (token only in the header):
   ...
   which validation failed, `401` = wrong token.
4. **Prove it's live:** fetch the returned page URL, confirm it renders, and show the
