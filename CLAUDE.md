...
```bash
# install (from repos/astra/, inside .venv)
pip install -e .

# start dev server
uvicorn astra.main:app --reload --port 8300
```

Default URL: `http://localhost:8300`.
...
- **Intranet-only; no GitHub.** No dependency on external package indexes at runtime; adopters are on Windows — install commands are PowerShell one-liners.
